...
_PRESERVE_COMPOUNDS = [
    ...
    "low-income", "high-income", "benefit-finding", "meaning-making",
...
def fix_hyphens(text: str) -> str:
    ...
    for w in _PRESERVE_COMPOUNDS:
        text = re.sub(re.escape(w), _record, text, flags=re.IGNORECASE)
...
_BOOK_SIGNS_RE = re.compile(
    r"\b(pp\.\s*\d|Eds\.|Ed\.\s|Editors|Publisher|Press[,:;\s]|"
    ...
    if result["is_book"]:
        ...
        if m_in:
            ...
            if len(parts) > 1:
                ...
                if m_pub:
                    result["journal"] = m_pub.group(1).strip()
                    if not result["year"]:
                        result["year"] = int(m_pub.group(2))
                else:
                    result["journal"] = pub_year.rstrip(" ,;.")
        result["parsing_confidence"] = "low"
        notes.append(
